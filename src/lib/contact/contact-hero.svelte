<script lang="ts">
	...
	const contactCards = [
		...
		{ icon: mdiEmailOutline, label: 'Email', value: 'abahansolution@gmail.com', href: 'mailto:abahansolution@gmail.com' },
		{ icon: mdiMapMarker, label: 'Address', value: 'Bhubaneswar, Odisha', href: 'https://maps.google.com/?q=Bhubaneswar,Odisha' }
	];
</script>
	...
	>
		<div class="grid gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
			<div class="space-y-6">
				...
				<h1 data-animate class="font-[var(--font-display)] text-4xl text-slate-900 sm:text-5xl">
				...
				<div data-animate class="grid gap-4 sm:grid-cols-3">
					{#each contactCards as card}
						<a
							href={card.href}
							class="group rounded-2xl border border-white/80 bg-white/90 p-4 text-sm text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:border-primary-200 hover:shadow-md"
						>
							<div class="flex items-center gap-3">
								<span
									class="flex h-10 w-10 items-center justify-center rounded-full bg-primary-50 text-primary-600 transition group-hover:scale-105"
								>
									<Icon path={card.icon} className="h-4 w-4" />
								</span>
								<div>
									<p class="text-xs font-semibold uppercase tracking-[0.25em] text-slate-400">{card.label}</p>
									<p class="mt-1 text-sm font-semibold text-slate-900">{card.value}</p>
								</div>
							</div>
						</a>
					{/each}
			...
			<div class="space-y-5">
				<div data-animate class="group relative overflow-hidden rounded-3xl border border-white/80 bg-white/90 shadow-lg">
					<img
				...
				<div data-animate class="rounded-3xl border border-white/80 bg-white/95 p-6 shadow-lg">
					...
					<div class="mt-4 grid gap-3 sm:grid-cols-2">
						...
						<div class="rounded-2xl border border-white/80 bg-white/90 px-4 py-3 text-center shadow-sm">
							...
							<p class="text-[0.6rem] font-semibold uppercase tracking-[0.2em] text-slate-500">Counselor support</p>
						</div>
					</div>
				</div>
			</div>
